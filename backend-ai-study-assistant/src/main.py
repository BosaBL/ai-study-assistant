...
async def update_firestore_document_success(doc_uuid: str, result: dict, metadata: dict) -> bool:
    ...
    try:
        ...
        db.collection(config.firestore_collection).document(doc_uuid).update(update_data)
        return True
    except Exception as e:
...
async def generate_bullet_points(llm, text_chunks: List[str]) -> List[BulletPoint]:
    ...
    try:
        ...
        if json_match:
            ...
            return [
                BulletPoint(point="Failed to parse bullet points", importance_level="high")
            ]
    except Exception as e:
        return [BulletPoint(point=f"Error generating bullet points: {str(e)}", importance_level="high")]
        ...
        if json_match:
            ...
            return [
                QuizQuestion(
                    ...
                    correct_answer="A", explanation="Generation failed"
                )
            ]
    except Exception as e:
        return [
            QuizQuestion(
                question=f"Error: {str(e)}", option_a="A", option_b="B",
                option_c="C", option_d="D", correct_answer="A", explanation="Error occurred"
            )
        ]
...
async def generate_flashcards(llm, text_chunks: List[str]) -> List[Flashcard]:
    ...
    try:
        ...
        if json_match:
            ...
            return [
                Flashcard(front="Sample flashcard failed", back="Generation failed", category="Error")
            ]
    except Exception as e:
        return [
            Flashcard(front=f"Error: {str(e)}", back="Generation failed", category="Error")
        ]
    ...
    try:
        ...
        for file_data in files_data:
            ...
            mock_file = type('MockUploadFile', (), {
                ...
                'read': lambda: asyncio.create_task(asyncio.coroutine(lambda: file_data['content'])())
            })()
        ...
        for file_data in files_data:
            try:
                ...
                for page in pdf_reader.pages:
                    pdf_text += page.extract_text() + "\n"
                full_text += f"=== Content from {file_data['filename']} ===\n{pdf_text}\n"
            except Exception as e:
...
async def process_pdfs(background_tasks: BackgroundTasks, files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to create tracking document")
    ...
    try:
        ...
        if doc_data["status"] == "finished" and doc_data.get("result"):
            ...
            response_data["result"] = ProcessingResponse(
                bullet_points=[BulletPoint(**bp) for bp in result_data["bullet_points"]],
                quiz_questions=[QuizQuestion(**qq) for qq in result_data["quiz_questions"]],
                flashcards=[Flashcard(**fc) for fc in result_data["flashcards"]],
                metadata=doc_data.get("metadata", {})
            )
        
        if doc_data["status"] == "error":
            response_data["error_message"] = doc_data.get("error_message", "Unknown error occurred")
...
async def list_summaries(limit: int = 50, status_filter: Optional[str] = None):
    ...
    try:
        query = db.collection(config.firestore_collection).order_by("created_at", direction=firestore.Query.DESCENDING).limit(limit)
        ...
        for doc in docs:
            ...
            if doc_data["status"] == "error":
                summary_info["error_message"] = doc_data.get("error_message", "Unknown error")
